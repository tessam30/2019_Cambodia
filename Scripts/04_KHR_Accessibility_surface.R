...
source(file.path(rpath, "03_KHR_basemaps.R")) # source this if not running in order
...
# If resampling is needed -- but not necessary for tiny ol' Cambodia
# KHR_travel_rs <- raster::aggregate(KHR_travel, fact = 1)


# Convert this into a data frame to plot w/ ggplot using geom_tile
KHR_travel_df <- as(KHR_travel, "SpatialPixelsDataFrame") %>%
  as.data.frame(.) %>%
...
KHR_accessiblity <-
  ...
  geom_sf(data = khr_nbrs, fill = "NA", colour = "#f9f9f9", size = 0.5, alpha = 0.85) + # mask neighbors
  
  geom_sf(data = khr_admin0, colour = "white", fill = "NA", size = 0.9) + # white stroke around KHR
  geom_sf(data = khr_admin0, colour = "black", fill = "NA") +
  geom_sf(data = khr_water2, fill = "#bee0ff", colour = "#8bc8ff", size = 0.5) +
  
  geom_sf_text_repel(data = ne_cities_trunc, aes(label = name), alpha = 0.90, family = "SegoeUI") + #text
  ...
  coord_sf(xlim = mapRange[c(1:2)], ylim = mapRange[c(3:4)]) + # limited extent
  labs(
    x = "", y = "",
  ...
  theme(
    ...
    plot.title = element_markdown(), # Fancy formatting for title
